...
function searchByWord() {
	...
	$.getJSON(
		"http://localhost:8080/MakeUp/search?word=" + val,
		function (data) {
...
function listByFilters() {
	...
	let requestString = "http://localhost:8080/MakeUp/getByFilters?";
...
function listAll() {
	$.getJSON("http://localhost:8080/MakeUp/getAll", function (data) {
		var table = document.getElementById("table");
...
function listBrands() {
	$.getJSON("http://localhost:8080/MakeUp/getBrands", function (data) {
	...
	for (let i = 0; i < data.length; i++) {
		...
		button.addEventListener("click", (e) => {
			if (
				...
				changePublicityOfProduct(e.target.value, false);
			}
		});
